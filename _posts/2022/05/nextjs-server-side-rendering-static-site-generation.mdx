...
Przewada `getStaticProps` w stosunku do `getInitialProps` polega jednak na tym, że nie trzeba nic więcej pobierać z bazy danych ani zewnętrznych API! Dane są już przecież gotowe – w postaci zapisanych w czasie budowania **statycznych plików JSON**. Genialne, prawda? A jakie proste.
